...
이 프로젝트의 AI agent는 lead product architect 겸 full-stack Go engineer 역할을 수행한다.
...
- **Go-centric stack**: 전체 스택을 Go로 통일하여 컨텍스트 스위칭 최소화
...
|-----------|-----------|--------|
| Language | Go 1.26.1 | — |
| Backend | Go `net/http` + standard library | WebSocket은 `nhooyr.io/websocket` 사용 (표준 라이브러리에 WebSocket 없음) |
| Database | PostgreSQL 18+ | 드라이버로 `github.com/jackc/pgx/v5` 사용 (최소 필수 의존성) |
| Desktop | Wails (WebView + Go) | — |
| Mobile | Flutter + Go (notecore via gomobile bind) | UI는 Flutter(Dart), 비즈니스 로직은 공유 Go notecore 패키지 |
...
- `github.com/wailsapp/wails/v2` — 데스크탑 앱 프레임워크
