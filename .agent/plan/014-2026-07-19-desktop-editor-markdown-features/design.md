...
- 렌더링: summary 줄에 ▸/▾ 토글 + 강조 스타일. (수정 2026-07-19) 에디터 내 본문 높이 접힘은 구현 중 실측으로 제외 — 스트럿 floor 제거가 미러 페인터 정렬(0.5px)을 깨뜨리는 것이 확인됨(plan 02 문서 결정 기록). 에디터에서는 본문을 항상 표시하고, 접힘은 open 속성을 통해 GitHub 웹/타 디바이스에서 동작. 에디터 내 시각 접힘은 후속 과제
