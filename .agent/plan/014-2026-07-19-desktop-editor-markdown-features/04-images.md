...
렌더링: 테이블 오버레이 패턴. img 태그 줄은 항상 투명 처리하되 첫 글자에 큰 fontSize를 줘 줄 높이를 이미지 표시 높이만큼 예약한다(스트럿은 최소값일 뿐이라 body 스트럿 그대로 동작 — 큰 글자가 줄을 키운다). 태그가 편집으로 깨지면 정규식 매칭이 풀려 원문 텍스트가 그대로 드러난다(자가 복구). width와 height를 둘 다 저장하는 이유: 이미지 바이트를 받기 전에 줄 높이를 예약해야 하기 때문.
