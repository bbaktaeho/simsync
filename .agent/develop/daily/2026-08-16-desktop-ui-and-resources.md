...
테스트가 통과했는데도 여전히 어긋나 보인다는 지적을 받았다. 위젯 테스트는
**텍스트 박스**를 재는데, 눈에 보이는 건 **잉크**다. 실행 중인 앱 스크린샷을
픽셀로 재서 확인했다:

| 대상 | 위 여백 | 아래 여백 | 편차 |
|------|---------|-----------|------|
| 힌트(수정 전) | 11.0px | 8.0px | +1.5px 아래 |
| 아이콘(수정 전) | 10.5px | 6.5px | +2.0px 아래 |

아이콘까지 함께 내려앉아 있었다 — 텍스트 메트릭 문제가 아니라 아이콘·글자 모두
잉크가 박스보다 아래에 그려지기 때문이다. `leadingDistribution: even`은 효과가
없었다(라인 박스가 잉크보다 작아 분배할 여유가 없다). 컨테이너 아래쪽에 4px
여백을 주는 광학 보정으로 잡았다:

| 대상 | 위 | 아래 | 편차 |
|------|-----|------|------|
| 힌트(수정 후) | 11.0px | 10.0px | +0.5px |
| 아이콘(수정 후) | 10.0px | 10.0px | 0.0px |

위젯 테스트는 박스 기준이라 임계값을 2.5px로 완화했다 — 예전처럼 7px씩 어긋나는
회귀를 잡는 역할로 남긴다. 정확한 정렬은 위 픽셀 측정이 근거다.
