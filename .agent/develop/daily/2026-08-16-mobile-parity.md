...
## 6. 데스크탑 UI/리소스 수정의 모바일 반영

데스크탑에서 고친 것들이 모바일에는 그대로 남아 있었다 (두 프로젝트가 코드를
복제하는 구조라 이런 누락이 생긴다). 대조해서 셋 다 반영했다.

- **동기화 폴링 ETag**: 모바일도 5초마다 GitHub 커밋 API를 친다. 모바일은
  데이터·배터리 비용이 직접 걸려 있어 이득이 더 크다. `If-None-Match`를 보내고
  304면 알던 sha를 유지한다(변경 콜백도 울리지 않는다). 테스트 추가.
- **마크다운 스타일시트**: `MarkdownStyleSheet.fromTheme(...).copyWith(...)`
  기반 공용 함수(`buildMarkdownStyleSheet`)로 통일. 지정하지 않은 슬롯이
  DefaultTextStyle에 의존해 화면마다 달라지던 것을 없앤다.
- **텍스트 대비**: 데스크탑과 같은 문제가 있었다 — `textMuted`(밝은 배경에서
  2.4~2.7:1)를 안내 문구·노트 개수·'저장 중'·빈 상태 설명에 쓰고 있었다.
  본문 성격 텍스트를 `textSecondary`로 올리고 대비 테스트를 추가했다.
- **검색창 테두리 제거**: 채운 배경만으로 충분하고, 포커스에서만 액센트
...
위클리·먼슬리 리뷰는 데스크탑이 **동기화 스토리지에 마크다운으로** 저장한다
(`notes/{YYYY-MM}/{N}주차/weekly-review.md`, `notes/{YYYY-MM}/monthly-review.md`).
스토리지 바이너리/텍스트 API를 이식해 둔 덕분에 모바일에서 그대로 읽을 수 있다.

- `review_paths.dart`, `review_service.dart`를 이식(읽기만 사용).
- `ReviewScreen` 신설: 월 이동, 먼슬리 + 주차별 리뷰 카드, 당겨서 새로고침.
  본문은 노트 미리보기와 같은 스타일시트로 렌더한다.
- 하단 탭에 **리뷰**를 추가 (노트 · 리뷰 · 검색 · 설정).
- 리뷰 **생성은 데스크탑 전용**이다. 빈 상태에서 그 사실을 안내한다.
- 리뷰 파일이 노트 목록에 섞이지 않는 것도 확인했다 — 모바일 스토리지가 숫자
  날짜 폴더만 노트로 인식한다.

검증: 뷰어 테스트 3개(렌더/빈 상태/공백 리뷰 제외), 모바일 168개 통과,
릴리즈 APK 재빌드.
