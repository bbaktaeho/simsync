...
- 작업을 시작하기 전에 반드시 .agent/guide.md를 읽어라.
...
- `.agent/`의 기존 아키텍처와 기술 선택은 역사적 컨텍스트로 취급해라. 고정된 진실이 아니다.
...
- [.agent/guide.md](.agent/guide.md) - 프로젝트 가이드
- [.agent/workflow.md](.agent/workflow.md) - 작업 워크플로우
- [DESIGN.md](DESIGN.md) - 디자인 시스템 (UI/UX 관련 작업 시 반드시 읽어라)

`AGENTS.md`는 이 리포지토리의 agent instruction single source of truth다.
도구별 instruction 파일은 `AGENTS.md`를 symlink로 가리킨다.
agent instruction은 여기에 간결하게 유지하고, 프로젝트 상세는 `.agent/`로 이동한다.
